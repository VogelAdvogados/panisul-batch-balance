...
import { RecipeWithIngredients, TablesInsert } from "@/integrations/supabase/types";
...
const RecipesTab = () => {
  ...
  const [formData, setFormData] = useState<Omit<TablesInsert<'recipes'>, 'id' | 'created_at' | 'updated_at'>>({
    name: "",
    description: "",
    yield_quantity: 1,
    yield_unit: "unidade"
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (recipeIngredients.length === 0 || recipeIngredients.some(ri => !ri.ingredient_id)) {
      toast({
    ...
    createRecipeMutation.mutate({
      ...
      onSuccess: () => {
        ...
        setFormData({ name: "", description: "", yield_quantity: 1, yield_unit: "unidade" });
    ...
    return recipes?.map((recipe) => {
      ...
      return (
        <TableRow key={recipe.id}>
          ...
          <TableCell>R$ {totalCost.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
          <TableCell>R$ {costPerUnit.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</TableCell>
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Preencha os detalhes da receita e adicione os ingredientes necessários.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="yield_quantity" type="number" step="0.01" value={formData.yield_quantity} onChange={(e) => setFormData({...formData, yield_quantity: Number(e.target.value) || 0})} required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="yield_unit">Unidade do Rendimento</Label>
                  <Input id="yield_unit" value={formData.yield_unit} onChange={(e) => setFormData({...formData, yield_unit: e.target.value})} placeholder="unidades, kg, etc" required />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button type="button" onClick={addIngredient} size="sm" variant="outline"><Plus className="h-4 w-4 mr-1" />Adicionar</Button>
                </div>
                    ...
                    <div key={index} className="flex gap-2 items-center">
                      <Select value={ingredient.ingredient_id} onValueChange={(value) => updateIngredient(index, "ingredient_id", value)}>
                        <SelectTrigger className="flex-1">
                          <SelectValue placeholder={isLoadingIngredients ? "Carregando..." : "Selecione um ingrediente"} />
                        </SelectTrigger>
                        <SelectContent>
                          {ingredients?.map((ing) => (
                      ...
                      <Input type="number" step="0.01" placeholder="Qtd" className="w-32" value={ingredient.quantity} onChange={(e) => updateIngredient(index, "quantity", parseFloat(e.target.value) || 0)} />
                      <Button type="button" variant="ghost" size="icon" onClick={() => removeIngredient(index)}><Trash className="h-4 w-4 text-red-500" /></Button>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Visualize todas as suas receitas e seus custos calculados.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
